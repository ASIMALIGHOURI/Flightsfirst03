...
(function () {
  ...
  /* Delivery: forms POST to FormSubmit (https://formsubmit.co) which relays
     ...
     The "Best time to call" field uses the browser's native required-field
     validation (empty disabled default option + required attribute), so the
     form cannot be submitted — and this handler cannot fire — until the
     visitor actively picks a time. */
